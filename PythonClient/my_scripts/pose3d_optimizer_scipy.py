...
class pose3d_flight_scipy():
    ...
    def forward(self, pose_3d):
        ...
        #projection
        queue_index = 0
        residuals_dict["proj"] = np.zeros([2* self.NUM_OF_JOINTS,])
        residuals_dict["smooth"] = np.zeros([3* self.NUM_OF_JOINTS,])
        residuals_dict["lift"] = np.zeros([3* (self.NUM_OF_JOINTS-1),])
        residuals_dict["bone"] = np.zeros([self.NUM_OF_JOINTS-1,])
        for bone_2d_, R_drone_, C_drone_ in self.data_list:

            #projection
            projected_2d, _ = take_bone_projection(pose_3d[queue_index, :, :], R_drone_, C_drone_)
            output["proj"] += mse_loss(projected_2d, bone_2d_)
            residuals_dict["proj"] += find_residuals(projected_2d, bone_2d_)
            ...
            if (queue_index != self.window_size-1 and queue_index != 0):
                ...
                residuals_dict["smooth"] += find_residuals(pose_3d[queue_index, :, :], pose_3d[queue_index+1, :, :]) + find_residuals(pose_3d[queue_index-1, :, :], pose_3d[queue_index, :, :])
            elif (queue_index != self.window_size-1 ):
                output["smooth"] += mse_loss(pose_3d[queue_index, :, :], pose_3d[queue_index+1, :, :])
                residuals_dict["smooth"] += find_residuals(pose_3d[queue_index, :, :], pose_3d[queue_index+1, :, :]) 
            elif (queue_index != 0):
                output["smooth"] += mse_loss(pose_3d[queue_index-1, :, :], pose_3d[queue_index, :, :])
                residuals_dict["smooth"] += find_residuals(pose_3d[queue_index-1, :, :], pose_3d[queue_index, :, :]) 
            ...
            residuals_dict["lift"] += find_residuals(pose3d_lift_directions, pose_est_directions) 
            ...
            residuals_dict["bone"] += bonelosses
        ...
        for ind, loss_key in enumerate(self.loss_dict):
            residuals_dict[loss_key] = np.squeeze(residuals_dict[loss_key])
            if ind == 0:
                residuals = residuals_dict[loss_key]
            else:
                residuals = np.concatenate([residuals, residuals_dict[loss_key]])
        return residuals