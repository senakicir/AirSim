...
def determineAllPositions(bone_pred, client):
    ...
    #TO DO;
    positions[HUMAN_POS_IND,:] = 0#TODODOOOOOO 
    return positions
...
def main():
    ...
    photo = TakePhoto(num_of_photos)
    bone_pred = FindJointPos2D(photo, num_of_photos, make_plot = False)
    initial_positions = determineAllPositions(bone_pred, client) #everything is relative to drone initial drone pos now (drone is at the center of coord. system) = AirSim coord. system
    ...
    if (USE_TRACKBAR == True):
        ...
        cv2.setTrackbarPos('Angle', 'Angle Control', int(degrees(some_angle)))
