...
class Trainer(object):
    ...
    def run_validation(self):
        ...
        for ti, test_function in enumerate(self.test_functions):
            ...
            if self.iteration % self.test_every[ti] != 0: # and self.iteration != 2000: # run test as specified and once after X iter
                continue
            ...
            if "loss" in test_output:
                ...
                if  (self.iteration>0 and ti >0) and (ti not in self.best_loss or test_output["loss"] < self.best_loss[ti]): # ti > 0, don't save for training loss
    ...
    def train(self, num_iters, print_every=0, maxtime=np.inf):
        ...
        if num_iters / self.save_every > max_snapshots:
            ...
            print("WARNING, instructed to save more then {} snapshots, reducing number of iterations to {}".format(max_snapshots, num_iters))
